...
type Systemd struct {
	...
	// Test seam; nil means exec.CommandContext.
	run func(ctx context.Context, args ...string) ([]byte, error)
	// available reports whether systemd is present.
	// Test seam; nil checks /run/systemd/system and PATH.
	available func() bool
}
...
// systemctl show exits 0 even for unknown units, so any error is a run failure.
